...
def _get_insert_clause_args(template):
    ...
    if template is None or template == {}:
        ...
        clause = " (" + ", ".join(columns) + ") VALUES (" + ", ".join("%s" for _ in columns) + ") "
...
def insert(db_schema, table_name, template):
    ic, args = _get_insert_clause_args(template)

    conn = _get_db_connection()
    cur = conn.cursor()

    sql = "insert into " + db_schema + "." + table_name + " " + ic
    res = cur.execute(sql, args=args)
